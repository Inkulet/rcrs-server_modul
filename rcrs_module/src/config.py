...
# Модуль конфигурации: единая точка хранения констант и настройки логирования.
# Все остальные модули импортируют значения отсюда, что исключает магические
# числа и дублирование констант по кодовой базе.
...
# Единый каталог для логов и метрик (по умолчанию <project_root>/log).
# Путь нормализуется в абсолютный и создаётся в setup_logging, что избавляет
# последующие writer-ы от проверки существования директории.
...
# Дополнительный штраф для same-role claim. Реализует простой ownership:
# цель, занятая агентом той же роли, почти всегда уступается, чтобы избежать
# дублирования работы. Legacy-claims без роли получают только базовый штраф.
SAME_ROLE_CLAIM_PENALTY: float = 0.7

# Срок памяти об услышанной цели союзника (FIRE_BRIGADE), в тактах.
# Позволяет разводить пожарных по разным очагам. Заменяет K-means
# кластеризацию из ADF простой «памятью» о занятых целях.
RECENT_ALLY_TARGET_TICKS: int = 15

# Дополнительный штраф (поверх CLAIMED_TARGET_PENALTY), применяемый к целям
# из recent_allies_targets для FIRE_BRIGADE.
...
# (10^6) даёт f_urgency ≈ 1e-6, что после clamp — всегда 0, и полиция
# ничего не выбирает. Деление на 1000 мм (1 м) приводит d' к диапазону
# [1..100] для типичных расстояний [1м..100м] и даёт осмысленное
# f_urgency ∈ [0.01..1.0].
...
# Срок (в тактах), после которого завал из skip-списка полиции вновь
# становится кандидатом на расчистку. Без этого таймаута ошибочно
# попавшие в список завалы оставались бы заблокированы навсегда.
